...
**Creation is the poweruser's, curation is the admin's.** SPEC section 2
grants the second role "label crops, add classes, read" in so many words (that
role is now named 'poweruser' — the rename amends the frozen SPEC, and the
poweruser additionally uploads frames), so `POST /api/classes` is open — a
taxonomy a poweruser cannot extend mid-session gets worked around by
mislabeling, which is far more expensive than a stray class. Renaming,
recoloring, archiving and restoring are not additive: they rewrite every
picker, every legend and every export's `data.yaml` for everyone at once, so
they sit behind the admin gate (SPEC section 2 grants admins "everything:
...
# ------------------------------------------------------------------ permissions
def test_poweruser_may_create_a_class(poweruser: TestClient):
    """SPEC section 2: powerusers add classes. Creation is additive and local —
    it costs a fresh yolo_index and changes nothing anyone already labeled."""
    resp = poweruser.post("/api/classes", json={"name": "wing"})
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert body["name"] == "wing"
    assert body["archived"] is False
    assert body["yolo_index"] == 0
    assert body["color"].startswith("#") and len(body["color"]) == 7


def test_poweruser_cannot_rename_a_class(poweruser: TestClient, mite_class: dict):
    ...
    resp = poweruser.patch(
        f"/api/classes/{mite_class['class_id']}", json={"name": "renamed"}
    )
    assert resp.status_code == 403
    assert resp.json()["detail"] == "admin only"


def test_poweruser_cannot_archive_a_class(poweruser: TestClient, mite_class: dict):
    ...
    resp = poweruser.delete(f"/api/classes/{mite_class['class_id']}")
    assert resp.status_code == 403


def test_poweruser_cannot_restore_a_class(
    ...
    assert admin.delete(f"/api/classes/{mite_class['class_id']}").status_code == 200
    resp = poweruser.post(f"/api/classes/{mite_class['class_id']}/restore")
    assert resp.status_code == 403


def test_admin_may_rename_recolor_and_describe(admin: TestClient, mite_class: dict):
    resp = admin.patch(
        f"/api/classes/{mite_class['class_id']}",
        json={"name": "dark debris", "color": "#123abc", "description": "on sheet"},
    )
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert body["name"] == "dark debris"
    assert body["color"] == "#123abc"
    assert body["description"] == "on sheet"
    # The id is what every mask references, so it survives the rename.
    assert body["class_id"] == mite_class["class_id"]
    assert body["yolo_index"] == mite_class["yolo_index"]


def test_a_duplicate_class_name_is_409(admin: TestClient, mite_class: dict):
...
# --------------------------------------------------------------------- archiving
def test_archive_hides_but_never_drops(admin: TestClient, mite_class: dict):
    ...
    assert row["name"] == mite_class["name"]
    assert row["yolo_index"] == mite_class["yolo_index"]


def test_restore_brings_a_class_back_unchanged(admin: TestClient, mite_class: dict):
    ...
    assert resp.json()["yolo_index"] == mite_class["yolo_index"]
...
def test_an_archived_name_stays_taken(admin: TestClient, mite_class: dict):
    ...
    admin.delete(f"/api/classes/{mite_class['class_id']}")
    resp = admin.post("/api/classes", json={"name": mite_class["name"]})
    assert resp.status_code == 409
...
def test_classes_are_ordered_by_yolo_index(admin: TestClient):
    """That order is the order of data.yaml and of the user's number keys
    ...
    assert _names(admin.get("/api/classes")) == ["mite", "varroa", "wax"]
...
def test_a_new_mask_on_an_archived_class_is_refused_with_a_useful_message(
    ...
    mask created against it would be invisible work — and the user needs to
    ...
    assert mite_class["name"] in detail
...
def test_archiving_keeps_existing_masks_countable(
    ...
    row = next(c for c in archived if c["class_id"] == mite_class["class_id"])
