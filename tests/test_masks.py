...
SPEC section 4: **soft delete everywhere.** Labeling hours are the only thing on
this box that cannot be regenerated, so `DELETE /api/masks/{id}` flags the row
and never removes it — a user who deletes the wrong polygon after twenty
...
polygons accepted (users draw them and the exporter does not care), no
...
# tracing an overlapping pair of instances with a mouse.
...
def test_create_returns_the_spec_mask_shape(
    ...
    mask = _post_mask(
        poweruser, crop_rows[0]["crop_id"], mite_class["class_id"], TRIANGLE
    )
    assert set(mask) == {
    ...
    assert mask["class_id"] == mite_class["class_id"]
...
def test_a_new_mask_shows_up_on_its_crop(
    ...
    mask = _post_mask(admin, crop_id, mite_class["class_id"], TRIANGLE)
...
def test_a_polygon_needs_three_vertices(
    ...
    resp = admin.post(
        "/api/masks",
        json={"crop_id": crop_rows[0]["crop_id"], "class_id": mite_class["class_id"],
...
def test_a_missing_polygon_is_400_not_422(
    ...
    """SPEC section 5 says a bad polygon is a 400 with a sentence the user
...
def test_self_intersecting_polygons_are_accepted(
    ...
    """SPEC section 3 is explicit: users make them and the exporter does not
    care. Rejecting them would block real work over a geometric nicety that
    nothing downstream ever checks."""
    mask = _post_mask(admin, crop_rows[0]["crop_id"], mite_class["class_id"], BOWTIE)
    assert mask["points"] == BOWTIE


def test_a_mask_on_an_unknown_crop_is_404(admin: TestClient, mite_class: dict):
    """404, not a 500. `db.get_crop` answers None and `api._need` is what turns
    that into a status code."""
    resp = admin.post(
        "/api/masks",
        json={"crop_id": "no_such_crop", "class_id": mite_class["class_id"],
              "points": TRIANGLE},
    )
...
def test_masks_on_a_deleted_images_crop_are_refused_cleanly(
    admin: TestClient, mite_class: dict, image: dict, crop_rows: list[dict]
):
    ...
    _post_mask(admin, crop_id, mite_class["class_id"], TRIANGLE)
...
def test_deleting_an_image_with_masks_needs_force(
    ...
    labeling hours, so the delete refuses until it is asked twice."""
    _post_mask(admin, crop_rows[0]["crop_id"], mite_class["class_id"], TRIANGLE)
...
def test_reshaping_a_mask_stamps_updated_at(
    ...
    crop = crop_rows[-1]
    mask = _post_mask(admin, crop["crop_id"], mite_class["class_id"], TRIANGLE)
...
def test_re_classing_a_mask_works(
    ...
    mask = _post_mask(admin, crop_rows[0]["crop_id"], mite_class["class_id"], TRIANGLE)
    ...
    assert counts[mite_class["class_id"]] == 0
...
def test_a_poweruser_may_label(
    poweruser: TestClient, mite_class: dict, crop_rows: list[dict]
):
    """Labeling crops is the poweruser's whole job (SPEC section 2; the role
    SPEC calls 'annotator' is renamed 'poweruser'), so create, edit, delete and
    complete all have to work without an admin."""
    crop_id = crop_rows[0]["crop_id"]
    mask = _post_mask(poweruser, crop_id, mite_class["class_id"], TRIANGLE)
    assert poweruser.patch(
        f"/api/masks/{mask['mask_id']}", json={"points": BOWTIE}
    ).status_code == 200
    assert poweruser.post(
